...
namespace FormCRUD
{
    public partial class Form1 : Form
    {
        ...
        {
            string connection = "server=localhost;user id=root;password=;database=crud";
            string query = "UPDATE tbl_barang SET nama='" + this.textBox3.Text + "',harga_jual='" + this.textBox5.Text + "',harga_beli='" + this.textBox4.Text + "',jml_barang='" + this.textBox7.Text + "',satuan='" + this.textBox6.Text + "' WHERE kode_barang=" + this.textBox2.Text + "'";
            MySqlConnection conn = new MySqlConnection(connection);
            MySqlCommand cmd = new MySqlCommand(query, conn);
            MySqlDataReader dr;
            conn.Open();
